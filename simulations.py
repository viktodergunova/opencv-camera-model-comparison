...
def get_image_points(images, apply_handheld=False, rotation_angle=5, max_translation=30):
    objpoints = []
    imgpoints = []
    for image in images:
        img = cv.imread(image)
        ...
        if apply_handheld:
            img = apply_handheld_simulation(img, rotation_angle, max_translation)
...
# HANDHELD SIMULATION
def apply_handheld_simulation(image, rotation_angle, max_translation, probability):
    if random.random() < probability:
        ...
        # returns normal image if img is not affected
        return image


def simulate_dist_params_with_handheld(objpoints, imgpoints, num_params, num_images_list, rotation_angle, max_translation, probability):
    def process_image_subset(num_images):
        noisy_imgpoints = []
        for img_path, pts in zip(all_images[:num_images], imgpoints[:num_images]):
            image = cv.imread(img_path)
            
            # apply randomly to images
            if random.random() < probability:
                image = apply_handheld_simulation(image, rotation_angle, max_translation, probability)
...
max_translation_range = (0, 30)  # max pixel translation in x and y direction
probability = 0.4
